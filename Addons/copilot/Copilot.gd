...
func revert_change():
	...
	if !code_edit or !request_code_state: return
...
func get_current_script():
	if !editor_interface: return null
...
func get_code_editor():
	...
	if base_editor:
		return base_editor.get_base_editor()
	return null
...
func request_completion():
	...
	if pre_post == ["", ""]:
		...
		push_error("Copilot.gd: Could not retrieve pre/post text for completion.")
		return
	var llm = get_llm()
	if !llm:
		remove_loading_indicator()
		push_error("Copilot.gd: Could not find active LLM node.")
		return
...
func get_pre_post():
	var editor = get_code_editor()
	if !editor:
		push_error("Copilot.gd: get_code_editor() returned null.")
		return ["", ""]
...
func get_llm():
	if !models.has(cur_model):
		push_error("Copilot.gd: No LLM path found for current model.")
		return null
